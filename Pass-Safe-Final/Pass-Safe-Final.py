...
if os.path.exists("accounts.json") == False: # Creates a json file containing the accounts if the file does not exist yet
        with open("accounts.json",'x+') as f:
            json.dump({},f) # Writes an empty dictionary inside the json
...
def create_account_name():
    while True:
        ...
        if check_name(account_name) == False: # Calls the check_name function to validate the name
            line()
            continue
